...
# ==============================================================================
# 4. Predictor Main Workflow (Training -> Inference)
# ==============================================================================
...
def main():
    parser = argparse.ArgumentParser(description="DAE Predictor V2.0 (Train on data.csv -> Predict on --csv)")
    parser.add_argument("--csv", type=str, required=True,
                        help="Path to the new CSV file containing molecules to predict.")
    parser.add_argument("--train_data", type=str, default="data.csv",
    ...
    df_train = try_read_csv(args.train_data)

    # Preprocess Training Data
    prep = MoleculePreprocessor()
    col_o = "open_smiles" if "open_smiles" in df_train.columns else "SMILES_o"
    col_c = "closed_smiles" if "closed_smiles" in df_train.columns else "SMILES_c"

    df_train["_SMI_O"] = [prep.canonicalize(s) for s in df_train[col_o]]
    df_train["_SMI_C"] = [prep.canonicalize(s) for s in df_train[col_c]]
    df_train = df_train.dropna(subset=["_SMI_O", "_SMI_C"]).reset_index(drop=True)

    # Label Processing (Log Scale)
    if "t_half_ms" in df_train.columns:
        y_train_raw = pd.to_numeric(df_train["t_half_ms"], errors='coerce') / 1000.0
    elif "log_t12" in df_train.columns:
        y_train_raw = 10 ** df_train["log_t12"]
    else:
        print("Error: Training data must have 't_half_ms' or 'log_t12' column.")
        return

    y_train = np.log10(y_train_raw.clip(1e-9, 1e18).values)
    print(f"   Training Samples: {len(df_train)}")

    # 1.1 Extract & Fit Deep Features (ChemBERTa + PCA)
    print("   -> Learning Deep Features (ChemBERTa + PCA)...")
    # 注意：这里我们提取特征并 fit PCA
    X_deep_tr_raw = extract_deep_features(df_train["_SMI_O"].tolist(), n_components=None)  # Get raw first
    # 重新实现 PCA fit 逻辑以保存对象
    from sklearn.preprocessing import StandardScaler
    scaler_deep = StandardScaler()
    X_deep_tr_std = scaler_deep.fit_transform(X_deep_tr_raw)

    pca = PCA(n_components=10)
    X_deep_tr = pca.fit_transform(X_deep_tr_std)
    print(f"      PCA Fit Done. Variance Explained: {np.sum(pca.explained_variance_ratio_):.2%}")

    # 1.2 Extract & Fit Physics Features
    print("   -> Learning Physics Features (HOMA/dQ)...")
    X_phys_tr = extract_physical_features(df_train)
    scaler_phys = StandardScaler()
    X_phys_tr_std = scaler_phys.fit_transform(X_phys_tr)

    # 1.3 Train Models
    print("   -> Training RF and GP models...")
    X_train_final = np.hstack([X_deep_tr, X_phys_tr_std])

    rf = RandomForestRegressor(n_estimators=100, random_state=42)
    rf.fit(X_train_final, y_train)

    gp = GaussianProcessRegressor(kernel=C(1.0) * Matern(1.0) + WhiteKernel(0.1), normalize_y=True)
    gp.fit(X_train_final, y_train)
    ...
    # Check columns
    t_col_o = "open_smiles" if "open_smiles" in df_target.columns else "SMILES_o"
    t_col_c = "closed_smiles" if "closed_smiles" in df_target.columns else "SMILES_c"

    if t_col_o not in df_target.columns or t_col_c not in df_target.columns:
        print(f"Error: Target CSV must contain '{t_col_o}' and '{t_col_c}'.")
        return

    # Preprocess Target
    print("   -> Preprocessing target molecules...")
    valid_indices = []
    clean_smi_o, clean_smi_c = [], []

    for idx, row in df_target.iterrows():
        so = prep.canonicalize(row[t_col_o])
        sc = prep.canonicalize(row[t_col_c])
        if so and sc:
            valid_indices.append(idx)
            clean_smi_o.append(so)
            clean_smi_c.append(sc)
        else:
            print(f"      Warning: Row {idx} invalid SMILES, skipping.")

    if not valid_indices:
        print("No valid molecules found in target CSV.")
        return

    # Create a temporary dataframe for feature extraction functions
    df_valid = pd.DataFrame({"_SMI_O": clean_smi_o, "_SMI_C": clean_smi_c})

    # 2.1 Target Features
    print("   -> Extracting Target Features...")
    # Deep features: Transform using established Scaler and PCA
    # Note: extract_deep_features inside V2 usually returns PCA-ed data.
    # We need raw embeddings here to apply the *training* PCA.
    # So we call the internal logic slightly differently or modify extract_deep_features.
    # To be safe and consistent with V2 code structure, we re-run extraction logic here manually for safety:

    # (Re-using tokenizer/model from global scope or re-init)
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    model = AutoModel.from_pretrained(MODEL_NAME)
    model.eval()

    target_embeddings = []
    for smi in tqdm(clean_smi_o, desc="      ChemBERTa Inference"):
        try:
            inputs = tokenizer(smi, return_tensors="pt", padding=True, truncation=True, max_length=128)
            with torch.no_grad():
                outputs = model(**inputs)
            target_embeddings.append(outputs.last_hidden_state[:, 0, :].numpy())
        except:
            target_embeddings.append(np.zeros((1, 768)))

    X_deep_target_raw = np.vstack(target_embeddings)
    X_deep_target_std = scaler_deep.transform(X_deep_target_raw)  # Use Training Scaler
    X_deep_target_pca = pca.transform(X_deep_target_std)  # Use Training PCA

    # Physics features: Transform using established Scaler
    X_phys_target = extract_physical_features(df_valid)
    X_phys_target_std = scaler_phys.transform(X_phys_target)  # Use Training Scaler

    # 2.2 Combine
    X_target_final = np.hstack([X_deep_target_pca, X_phys_target_std])

    # 2.3 Predict
    print("   -> Generating Predictions...")
    pred_rf = rf.predict(X_target_final)
    pred_gp = gp.predict(X_target_final)

    # Ensemble Average (or just GP if it was better, but V2 used both in CV)
    # Let's align with the V2 philosophy: GP refines, RF is robust. Average is safe.
    pred_log_t12 = (pred_rf + pred_gp) / 2.0
    pred_t12_s = 10 ** pred_log_t12

    # 2.4 Save Results
    results_df = df_target.copy()
    # Fill predictions only for valid rows
    results_df["Predicted_log_t12"] = np.nan
    results_df["Predicted_t12_s"] = np.nan

    results_df.loc[valid_indices, "Predicted_log_t12"] = pred_log_t12
    results_df.loc[valid_indices, "Predicted_t12_s"] = pred_t12_s

    output_filename = "V2_Predictions.csv"
    ...
    print("=" * 50)
    print(results_df[[t_col_o, "Predicted_t12_s"]].head())
